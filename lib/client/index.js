...
function WorkerPouch(opts, callback) {
  //TODO make it fail safe for future cases
  if (!window.__pouchdb_global_worker) window.__pouchdb_global_worker = {};
  if (opts.worker_thread) var worker = window.__pouchdb_global_worker[opts.worker_thread];
  ...
  if (!worker) {
    try {
      ...
      window.__pouchdb_global_worker[opts.worker_thread] = worker;
    } catch (e) {
